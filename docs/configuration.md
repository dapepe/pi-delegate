...
| `max_turns` / `max_tool_calls` | `12` / `60` per worker |
| `timeout_seconds` | `600` per worker |
| `submit_grace_seconds` | `120`; must be shorter than the timeout. In the final window only `submit_result` is accepted, and every tool result warns from twice that window, so a worker that has finished reading submits instead of timing out |
...
| `timeout` | Hit `timeout_seconds` without submitting; the hint says whether the wall clock went to per-request latency (mean request time is reported) or to the packet | `provider` when requests averaged a minute or more, else `packet` |
...
| `context_limit`, `turn_limit`, `tool_limit` | A runner ceiling was reached | `packet` |
...
The suggested kind is a hint for the host's assessment vocabulary (`none`, `model`, `provider`, `packet`, `host`, `unknown`); the host decides what to record. `timing` (request count, mean and maximum request seconds) and `deadline` (warnings shown, tool calls refused in the grace window) are stored beside it and rendered in `report.md`.
